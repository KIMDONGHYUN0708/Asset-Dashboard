      ...
      <div className="fixed inset-0 z-50 bg-slate-950/90 backdrop-blur-sm flex items-center justify-center p-4">
        <div className="bg-slate-900 border border-slate-700 rounded-2xl p-8 max-w-md w-full shadow-2xl">
          <div className="flex items-center gap-3 mb-5">
            ...
            <div>
              <h2 className="text-[15px] font-bold text-white">나만의 데이터 키 설정</h2>
          ...
          <div className="space-y-4">
            ...
            <div className="p-4 rounded-xl bg-slate-800 border border-slate-700">
              <p className="text-[12px] font-medium text-white mb-1">처음 사용하시나요?</p>
              <p className="text-[11px] text-slate-500 mb-3">
                키를 생성하면 자산 입력 화면으로 이동합니다. 키는 반드시 안전한 곳에 보관하세요.
              </p>
              <div className="flex gap-2">
                <input
                  className="flex-1 px-3 py-2 text-[12px] bg-slate-700 border border-slate-600 rounded-lg text-white placeholder:text-slate-500 font-mono"
                  value={inputKey}
                ...
                <button onClick={generateKey}
                  className="px-3 py-2 text-[12px] font-medium text-white bg-slate-600 hover:bg-slate-500 border border-slate-500 rounded-lg flex items-center gap-1.5 transition-colors flex-shrink-0">
                  <RefreshCw size={12} /> 생성
                </button>
              </div>
              {inputKey && (
                <button onClick={() => applyKey(inputKey)}
                  className="mt-2 w-full py-2 text-[13px] font-semibold text-white bg-blue-500 hover:bg-blue-600 rounded-lg transition-colors">
                  이 키로 시작하기 →
                </button>
              )}
            </div>
            ...
            {/* 기존 키 입력 */}
            <div className="p-4 rounded-xl bg-slate-800 border border-slate-700">
              <p className="text-[12px] font-medium text-white mb-2">기존 키가 있으신가요?</p>
              <div className="flex gap-2">
