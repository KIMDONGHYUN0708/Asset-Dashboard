...
const SECTOR_COLORS: Record<string, { bg: string; border: string; text: string; bar: string }> = {
  ...
  '기타':      { bg: 'bg-slate-700/20', border: 'border-slate-700/40',  text: 'text-slate-400',  bar: 'bg-slate-500' },
};
  ...
  return (
    <div className="rounded-2xl bg-slate-900 border border-slate-800 p-5">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h2 className="text-[13px] font-semibold text-white">섹터 포트폴리오</h2>
          <p className="text-[11px] text-slate-500 mt-0.5">현재 스냅샷 · {items.length}종목</p>
        </div>
        <span className="text-[12px] font-semibold text-slate-300 tabular-nums">{formatKRW(total)}</span>
      </div>
        ...
        {data.map((d) => {
          ...
          return (
            ...
            >
              ...
              <div>
                <p className="text-[15px] font-bold text-white tabular-nums leading-tight">
                  {formatKRW(d.value)}
                </p>
