...
export default function DataKeySetupPanel() {
  ...
  return (
    <div className="max-w-lg space-y-5">
      {/* 현재 키 */}
      {uuid ? (
        <div className="rounded-2xl bg-slate-800/60 border border-slate-700 p-5 space-y-4">
          <div className="flex items-center gap-2">
            <ShieldCheck size={14} className="text-emerald-400" />
            <h3 className="text-[13px] font-semibold text-white">내 데이터 키</h3>
          ...
          <div className="flex gap-2">
            <code className="flex-1 px-3 py-2 bg-slate-900 border border-slate-700 rounded-lg font-mono text-[11px] text-slate-300 truncate">
              {uuid}
            </code>
          ...
          <div className="flex gap-2">
            <button onClick={handleSyncNow}
              className={`flex-1 py-2 text-[12px] font-medium rounded-lg border transition-colors flex items-center justify-center gap-1.5 ${
                ...
                : 'text-slate-400 border-slate-700 hover:text-white bg-slate-800'
              }`}>
              <ShieldCheck size={12} />
        ...
        <div className="rounded-2xl bg-slate-800/60 border border-dashed border-slate-600 p-5 text-center">
      ...
      <div className="rounded-2xl bg-slate-900 border border-slate-800 p-5 space-y-3">
        <div className="flex items-center gap-2 mb-1">
          ...
          <h3 className="text-[13px] font-semibold text-white">새 키 생성</h3>
        </div>
        ...
        <div className="flex gap-2">
          <input readOnly value={inputKey}
            ...
            className="flex-1 px-3 py-2 text-[11px] bg-slate-800 border border-slate-700 rounded-lg text-slate-300 font-mono placeholder:text-slate-600 focus:outline-none" />
          <button onClick={generateKey}
            className="px-3 py-2 text-[12px] font-medium text-white bg-slate-700 hover:bg-slate-600 border border-slate-600 rounded-lg flex items-center gap-1.5 transition-colors flex-shrink-0">
            <RefreshCw size={12} /> 생성
          </button>
        </div>
        {inputKey && (
          <button onClick={() => applyKey(inputKey)}
            className="w-full py-2 text-[13px] font-semibold text-white bg-blue-500 hover:bg-blue-600 rounded-lg transition-colors">
            이 키로 적용하기
          </button>
        )}
      </div>
      ...
      <div className="rounded-2xl bg-slate-900 border border-slate-800 p-5 space-y-3">
        <div className="flex items-center gap-2 mb-1">
          ...
          <h3 className="text-[13px] font-semibold text-white">기존 키 입력</h3>
        </div>
        <p className="text-[11px] text-slate-500">다른 기기에서 사용하던 키를 입력하면 해당 데이터를 불러옵니다.</p>
        <div className="flex gap-2">
          <input id="existing-key"
            placeholder="xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"
            className="flex-1 px-3 py-2 text-[11px] bg-slate-800 border border-slate-700 rounded-lg text-slate-300 font-mono placeholder:text-slate-600 focus:outline-none focus:border-blue-500/60" />
          <button onClick={() => {
            const v = (document.getElementById('existing-key') as HTMLInputElement)?.value;
            if (v) applyKey(v);
          }}
            className="px-3 py-2 text-[12px] font-medium text-white bg-blue-500/20 hover:bg-blue-500/30 border border-blue-500/30 rounded-lg flex items-center gap-1.5 transition-colors flex-shrink-0">
