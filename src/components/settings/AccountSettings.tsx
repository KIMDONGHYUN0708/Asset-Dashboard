...
const CAT_INSTITUTIONS = [...BANK_LIST, ...SECURITIES_LIST, ...INSURANCE_LIST, '기타'];

// 로고 파일이 있는 기관 (public/logos/ 기준)
const LOGO_MAP: Record<string, string> = {
  ...
  '토스':         '/logos/토스.svg',  // 기존 데이터 호환
  '토스뱅크':     '/logos/토스.svg',
  '삼성증권':     '/logos/삼성.svg',
  '삼성생명':     '/logos/삼성.svg',
  '한국투자증권': '/logos/한국투자.svg',
  '한국투자':     '/logos/한국투자.svg', // 기존 데이터 호환
  '한화투자증권': '/logos/한화.svg',
  '한화생명':     '/logos/한화.svg',
  '교보증권':     '/logos/교보.svg',
  '교보생명':     '/logos/교보.svg',
...
export default function AccountSettings() {
  ...
  const saveAll = () => {
    const updated = accounts.map(a =>
      editing[a.id]
        ? { ...editing[a.id], logo: LOGO_MAP[editing[a.id].institution] ?? undefined }
        : a
    );
          ...
          <div className="p-4 rounded-xl border border-blue-500/30 bg-blue-500/5 space-y-3 mb-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                >
                  <optgroup label="은행">
                    {BANK_LIST.map(i => <option key={i}>{i}</option>)}
                  </optgroup>
                  <optgroup label="증권사">
                    {SECURITIES_LIST.map(i => <option key={i}>{i}</option>)}
                  </optgroup>
                  <optgroup label="보험사">
              ...
              <div>
                ...
                <Select
                  ...
                  onChange={e => setNewItem(p => ({ ...p, category: e.target.value as AssetCategory }))}
                >
                ...
                <>
                  <div>
                    ...
                    <Input
                      ...
                      onChange={e => setNewItem(p => ({ ...p, interestRate: e.target.value ? Number(e.target.value) : undefined }))}
                      placeholder="3.5"
                    />
                  </div>
                  <div>
                    <label className="text-xs text-slate-500 mb-1 block">
                      만기일 <span className="text-slate-600">— 선택</span>
                    </label>
                    {noMaturity ? (
                      <div className="flex items-center gap-2 h-9">
                        <span className="text-xs text-slate-400">만기일 없음 (자유/청약)</span>
                        <button
                          onClick={() => setNoMaturity(false)}
                      ...
                      <div className="flex gap-1.5">
                        <Input
                          type="date"
                          value={newItem.maturityDate ?? ''}
                          onChange={e => setNewItem(p => ({ ...p, maturityDate: e.target.value || undefined }))}
                        />
                        <button
              ...
              {newItem.category === 'loan' && (
                <div>
                  ...
                  <Input
                    type="number" step="0.1"
                    value={newItem.loanRate ?? ''}
                    onChange={e => setNewItem(p => ({ ...p, loanRate: e.target.value ? Number(e.target.value) : undefined }))}
                    placeholder="5.2"
                  />
                </div>
              )}
            ...
            <div className="flex items-center gap-2 pt-1">
              ...
              <span className="text-xs text-slate-400">{newItem.institution} 로고 미리보기</span>
            </div>
            <div className="flex gap-2">
              <button
                onClick={addItem}
              ...
              <button
                onClick={() => { setAdding(false); setNoMaturity(false); setNewItem(blankAccount()); }}
                className="px-4 py-1.5 text-slate-400 text-sm rounded-lg hover:bg-slate-800 transition-colors"
              >
                취소
              </button>
            </div>
              ...
              <div className="space-y-2">
                {catItems.map(acc => {
                  ...
                  return (
                    ...
                    >
                      <InstitutionLogo name={cur.institution} />
                        ...
                        <div className="flex-1 grid grid-cols-2 md:grid-cols-4 gap-2">
                          <div>
                            ...
                            <Select
                              ...
                              onChange={e => updateEdit(acc.id, 'institution', e.target.value)}
                            >
                              <optgroup label="은행">
                                {BANK_LIST.map(i => <option key={i}>{i}</option>)}
                              </optgroup>
                          ...
                          <div>
                            <label className="text-xs text-slate-500 block mb-0.5">잔액</label>
                            <Input
                              type="number"
                              value={ed.amount}
                          ...
                          {cat === 'savings' && (
                            <>
                              <div>
                                ...
                                <Input
                                  type="number" step="0.1"
                                  value={ed.interestRate ?? ''}
                                  onChange={e => updateEdit(acc.id, 'interestRate', e.target.value ? Number(e.target.value) : undefined)}
                                />
                              </div>
                              <div>
                                ...
                                <Input
                                  type="date"
                                  value={ed.maturityDate ?? ''}
                                  onChange={e => updateEdit(acc.id, 'maturityDate', e.target.value || undefined)}
                                />
                              </div>
                            </>
                          )}
                          {cat === 'loan' && (
                            <div>
                              ...
                              <Input
                                type="number" step="0.1"
                                value={ed.loanRate ?? ''}
                                onChange={e => updateEdit(acc.id, 'loanRate', e.target.value ? Number(e.target.value) : undefined)}
                              />
                            </div>
                          )}
                        </div>
